...
class SalesView(ttk.Frame):
    def __init__(self, parent):
        ...
        self.cart_tree.bind("<Double-1>", self.edit_cart_item_price_event) # <-- AÑADIR EVENTO
    ...
    def add_to_cart_event(self, event):
        ...
        if product_stock <= 0:
            messagebox.showwarning("Sin Stock", "Este producto no tiene stock.", parent=self); return
    ...
    def edit_cart_item_price_event(self, event):
        """Pide un nuevo precio para el item seleccionado en el carrito."""
        ...
        new_price = simpledialog.askfloat("Editar Precio", f"Ingrese el nuevo precio de venta para:\n{product_name}", 
                                           initialvalue=current_price, parent=self, minvalue=0.0)
        if new_price is not None:
            self.controller.update_cart_item_price(selected_item_id, new_price)
